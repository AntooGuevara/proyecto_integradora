...
class RentasModel:
    ...
    #-----------------------------------------------------------------------------------
#obtener_nombres_articulos
    def obtener_nombres_clientes(self):
        """Obtiene nombres e IDs de clientes activos desde la BD."""
        cursor = self.obtener_cursor()
        if not cursor:
            return []
        
        try:
            # Consulta simple que funciona con tu estructura de BD
    ...
    def obtener_nombres_articulos(self):
        """Obtiene nombres e IDs de artículos activos y disponibles desde la BD real."""
        cursor = self.obtener_cursor()
        if not cursor:
            print("ERROR: No se pudo obtener cursor para artículos")
            return []
        
        try:
            print("DEBUG: Consultando tabla 'articulos'...")
            ...
            try:
                ...
                if not tabla_existe:
                    print("DEBUG: La tabla 'articulos' NO existe en la BD")
                    return []
                print("DEBUG: La tabla 'articulos' existe")
            except:
                print("DEBUG: No se pudo verificar existencia de tabla 'articulos'")
            
            # Intenta diferentes consultas
            consultas = [
                # Opción 1: Con filtros activo y disponible
                """
                SELECT id, nombre 
                FROM articulos 
                ...
                ORDER BY nombre
                """,
                # Opción 3: Sin filtros
                """
                SELECT id, nombre 
                FROM articulos 
                ORDER BY nombre
                """,
                # Opción 4: Consulta mínima
                """
                SELECT id, 'Artículo ' || id as nombre 
                FROM articulos 
                LIMIT 10
                """
            ]
            ...
            for i, consulta in enumerate(consultas):
                try:
                    print(f"DEBUG: Intentando consulta {i+1} para artículos...")
                    cursor.execute(consulta)
                    resultado = cursor.fetchall()
                    
                    if resultado:
                        print(f"DEBUG: Consulta {i+1} exitosa. Artículos encontrados: {resultado}")
                        articulos = resultado
                        break
                    else:
                        print(f"DEBUG: Consulta {i+1} no devolvió resultados")
                except Error as e:
                    print(f"DEBUG: Error en consulta {i+1}: {e}")
                    continue
            ...
            for articulo in articulos:
                if isinstance(articulo, dict):
                    ...
                    id_articulo = articulo.get('id')
                    nombre_articulo = articulo.get('nombre', f'Artículo {id_articulo}')
                    
                    resultado_final.append({
                        'id': id_articulo,
                        'nombre': nombre_articulo
                    })
                elif isinstance(articulo, (list, tuple)) and len(articulo) >= 2:
                    # Si es tupla o lista
                    resultado_final.append({
                        'id': articulo[0],
                        'nombre': articulo[1] if len(articulo) > 1 else f'Artículo {articulo[0]}'
                    })
                else:
                    print(f"DEBUG: Formato inesperado en artículo: {articulo}")
            
            print(f"DEBUG: Artículos formateados: {resultado_final}")
            return resultado_final
            
        except Error as e:
            print(f"ERROR obteniendo nombres de artículos: {e}")
            return []
        finally:
            cursor.close()
    
    # obtener_nombres_clientes
...
#_show_ordenes
# En model.py, dentro de la clase RentasModel

    def obtener_detalles_orden_completa(self, orden_id):
        ...
        try:
            ...
            # También obtener los artículos de esta orden
            if orden:
                query_articulos = """
                    SELECT dr.*, a.nombre as articulo_nombre
                    FROM detalle_reservacion dr
                    LEFT JOIN articulos a ON dr.articulo_id = a.id
                    WHERE dr.reservacion_id = %s
                """
                cursor.execute(query_articulos, (orden_id,))
                orden['articulos'] = cursor.fetchall()
            
            return orden
            
        except Error as e:
            print(f"Error obteniendo detalles de orden: {e}")
            return None
        finally:
            cursor.close()
    ...
    def obtener_todas_las_reservaciones(self):
        ...
        print("=== DEBUG: obtener_todas_las_reservaciones ===")
        ...
        try:
            query = """
                SELECT r.id, c.nombre AS cliente, r.estado, 
                    r.fecha_evento, r.fecha_entrega, r.total, r.observaciones
                FROM reservaciones r
                LEFT JOIN clientes c ON r.cliente_id = c.id
                ORDER BY r.fecha_reservacion DESC
            """
            ...
            print(f"DEBUG: {len(rows)} filas obtenidas de la BD")
            ...
            for r in rows:
                reservacion = {
                    ...
                    "cliente": r.get('cliente') or "",
                    "estado": r.get('estado') or "",
                    "fecha_entrega": str(r.get('fecha_entrega'))[:10] if r.get('fecha_entrega') else "",
                    "total": float(r.get('total')) if r.get('total') else 0.0
                }
                print(f"  ID: {reservacion['id']}, Cliente: {reservacion['cliente']}, Artículo: {reservacion['articulo'][:20]}...")
